...
def get_or_create_database(token: str) -> int:
    ...
    for db in databases:
        if db.get("name") == DB_LABEL:
            print(
                f"Database '{DB_LABEL}' already connected (id={db['id']}).", flush=True
            )
            return db["id"]
    ...
    for d in dashboards:
        if d.get("name") == DASHBOARD_NAME:
            ...
            if len(detail.get("dashcards", [])) >= len(QUESTIONS):
                ...
                return
            print(
                f"Dashboard '{DASHBOARD_NAME}' exists but has no cards — adding them …",
                flush=True,
            )
...
def main():
    ...
    print(
        f"\nDone — open {METABASE_URL} and go to Dashboards → '{DASHBOARD_NAME}'",
        flush=True,
    )
